...
from enum_multi import PHASE, MOD, COMPONENTS
import utils_data
import random
...
if (MODALITY == 'img' or MODALITY == 'multimodal' or MODALITY == 'InfoNCE_supervised'):
	MODALITY = MOD.img
...
testing_set_bag = Dataset_instance(test_dataset, PHASE.valid, 0.0, N_CLASSES, possible_reports = [], components = COMPONENTS_TO_USE, CNN_TO_USE = CNN_TO_USE)
...
y_pred = np.empty((len(test_dataset)), dtype=np.uint8)
y_true = np.empty((len(test_dataset)), dtype=np.uint8)

cumulative_preds = np.empty((len(test_dataset), N_CLASSES))
...
for i in progress_bar:
	...
	with torch.autocast(device_type='cuda', dtype=torch.float16):
		...
		try:
			IDs, tokens, labels, _, batch_filenames = next(dataloader_iterator)
		...
		IDs = torch.as_tensor(IDs).float().to(device, non_blocking=True)
		...
		if (MODALITY is MOD.img):
			for j in range(len(outputs_np_img)):
				cont = int(i * BATCH_SIZE + j)
				cumulative_preds[cont] = outputs_np_img[j]

		elif (MODALITY is MOD.txt):
			for j in range(len(outputs_np_txt)):
				cont = int(i * BATCH_SIZE + j)
				cumulative_preds[cont] = outputs_np_txt[j]

		if (i%10 == 0):
				
			try:
				kappa_val = kappa_score(y_true[:cont], y_pred[:cont], None, None, None, None)
				_,_,f1_score_val = f1_scores(y_true[:cont], y_pred[:cont], None, None, None, None)
...
if (MODALITY is MOD.img):
	utils_data.save_prediction(checkpoint_path, N_CLASSES, phase_str, None, preds, None, DATASET, MODALITY)
else:
