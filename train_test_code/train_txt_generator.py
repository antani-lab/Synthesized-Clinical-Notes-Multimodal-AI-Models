...
pre_trained_model = 'PLACEHOLDER/model_weights/multimodal/' + MODALITY + '/'
...
train_dataset = utils_data.get_instances_txt_generation(MAIN_FOLDER, list_DATASETS_train, flag_train, CNN_TO_USE, MODALITY, N_EXP_str, CSV_FOLDER, REPORTS_TRAINING = REPORTS_TRAINING, flag_KEYWORDS = flag_KEYWORDS)
valid_dataset = utils_data.get_instances_txt_generation(MAIN_FOLDER, list_DATASETS_valid, flag_valid, CNN_TO_USE, MODALITY, N_EXP_str, CSV_FOLDER, REPORTS_TRAINING = REPORTS_TRAINING, flag_KEYWORDS = flag_KEYWORDS)
...
"""
fname_semi_supervised = OUTPUT_folder + 'multimodal/'+MODALITY+'/multiclass/'+CNN_TO_USE+'/N_EXP_'+N_EXP_str+'/weakly_alignment/labels_train.csv'

csv_additional = pd.read_csv(fname_semi_supervised, sep = ',', header = None).values

train_dataset = np.vstack((train_dataset, csv_additional))  # shape (8, 2)
"""
...
if (MODALITY is MOD.CLIP):
	pre_trained_encoder = MultimodalArchitecture_CLIP(device, CNN_TO_USE, in_dim = input_dim, 
						out_dim = output_dim, 
