...
parser.add_argument('-w', '--weights', help='algorithm for pre-trained weights (PanDerm)',type=str, default='')
...
train_dataset = utils_data.get_instances_paths_from_bags(DATA_FOLDER, list_DATASETS_train, flag_train, CSV_FOLDER)
valid_dataset = utils_data.get_instances_paths_from_bags(DATA_FOLDER, list_DATASETS_train, flag_valid, CSV_FOLDER)

if (MODALITY is not MOD.img and REPORT_AUGMENTATION == False):
	flag_embeddings_train = utils_data.get_flag_embeddings_exist(train_dataset[:,2], REPORTS[REPORTS_TRAINING])
	flag_embeddings_valid = utils_data.get_flag_embeddings_exist(train_dataset[:,2], REPORTS[REPORTS_TRAINING])

	print("flag_embeddings_train: " + str(flag_embeddings_train))
	print("flag_embeddings_valid: " + str(flag_embeddings_valid))
else:
...
labels = train_dataset[:, 1].astype(int)
class_counts = np.bincount(labels)               # [#class0, #class1, ...]
class_weights = 1.0 / (class_counts + 1e-6)      # avoid div by zero
sample_weights = class_weights[labels]           # assign to each sample

sample_weights_tensor = torch.DoubleTensor(sample_weights)

sampler = WeightedRandomSampler(
	weights=sample_weights_tensor,
	num_samples=len(sample_weights_tensor),  # draw as many as dataset size
	replacement=True                         # important: allows oversampling
)
...
training_set_bag = Dataset_instance_concept(train_dataset, PHASE.train, prob, N_CLASSES, possible_reports, flag_augment_reports = REPORT_AUGMENTATION, components = COMPONENTS_TO_USE, flag_embeddings_pubmed = flag_embeddings_train, CNN_TO_USE = CNN_TO_USE)
training_generator_bag = data.DataLoader(training_set_bag, **params_train_bag)

validation_set_bag = Dataset_instance_concept(valid_dataset, PHASE.valid, 0.0, N_CLASSES, possible_reports, flag_augment_reports = False, components = COMPONENTS_TO_USE, flag_embeddings_pubmed = flag_embeddings_valid, CNN_TO_USE = CNN_TO_USE)
...
elif (MODALITY is MOD.NT_Xent_InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE):
	TEMPERATURE_NT_Xent = 0.5
...
model = MultimodalArchitecture(device, CNN_TO_USE, in_dim = input_dim, 
						...
						TEMPERATURE = TEMPERATURE, pretrained_path=model_weights_filename_pre_trained_img, patch_size=16, panderm_path = PANDERM_FOLDER)
...
elif (MODALITY is MOD.NT_Xent_InfoNCE_supervised):

	criterion_NT_Xent = loss_functions.NT_Xent_supervised(batch_size = BATCH_SIZE, temperature = TEMPERATURE_NT_Xent)
	criterion_InfoNCE = loss_functions.InfoNCE_supervised(temperature = TEMPERATURE_InfoNCE)

elif (MODALITY is MOD.NT_Xent_InfoNCE):

	criterion_NT_Xent = loss_functions.NT_Xent(batch_size = BATCH_SIZE, temperature = TEMPERATURE_NT_Xent)
	criterion_InfoNCE = loss_functions.InfoNCE(temperature = TEMPERATURE_InfoNCE)
...
def model_forward(epoch, phase = 'train'):
	...
	representation_nt_xent = 0.0
	representation_infonce = 0.0
	...
	for i in progress_bar:
		...
		with torch.autocast(device_type='cuda', dtype=torch.float16):
			...
			if (phase is PHASE.train):
				if (MODALITY is MOD.img):
				...
				elif (MODALITY is MOD.multimodal or MODALITY is MOD.InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE_supervised ):
					IDs = IDs.to(device)
					tokens = tokens.to(device)
					...
					logits_img, cls_img, logits_txt, cls_txt = model(IDs, tokens, flag_embeddings_pubmed)
		  			...
		  			MODALITY is MOD.CLIP_only or MODALITY is MOD.NT_Xent_only or MODALITY is MOD.InfoNCE_only or
					MODALITY is MOD.NT_Xent_InfoNCE
					):
					IDs = IDs.to(device)
					tokens = tokens.to(device)
				...
				with torch.no_grad():
					...
					if (MODALITY is MOD.img):
						...
						or MODALITY is MOD.NT_Xent_InfoNCE):
						IDs = IDs.to(device)
						tokens = tokens.to(device)
						_, cls_img, _, cls_txt = model(IDs, tokens, flag_embeddings_pubmed)
			...
			if (MODALITY is MOD.img or MODALITY is MOD.multimodal or MODALITY is MOD.InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE_supervised):
				if (N_CLASSES > 1):
			...
			if (MODALITY is MOD.txt or MODALITY is MOD.multimodal or MODALITY is MOD.InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE_supervised):
				if (N_CLASSES > 1):
			...
			if (MODALITY is MOD.img or MODALITY is MOD.txt or MODALITY is MOD.multimodal or MODALITY is MOD.InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE_supervised):
				for j in range(len(labels_np)):
					cont = int(i * BATCH_SIZE + j)
					y_true[cont] = labels_np[j]

			if (i%50 == 0 and (MODALITY is MOD.img or MODALITY is MOD.multimodal or MODALITY is MOD.InfoNCE_supervised or MODALITY is MOD.NT_Xent_InfoNCE_supervised)):
			...
			if (MODALITY is MOD.img):
				...
				if (phase is PHASE.train):
					...
					loss = loss_img + loss_txt + loss_representation_cosine + lambda_loss * loss_contrastive + loss_representation_rmse
				...
				try:
					loss_nt_xent = criterion_NT_Xent(cls_img, cls_txt, labels)
					loss_infonce = criterion_InfoNCE(cls_img, cls_txt, labels)
					loss_contrastive = loss_nt_xent + loss_infonce
					representation_contrastive = representation_contrastive + ((1 / (i+1)) * (loss_contrastive.item() - representation_contrastive))
				except Exception as e:
					print(e)
					loss_contrastive = 0.0
				...
				try:
					loss_nt_xent = criterion_NT_Xent(cls_img, cls_txt)
					loss_infonce = criterion_InfoNCE(cls_img, cls_txt)
					loss_contrastive = loss_nt_xent + loss_infonce
					representation_contrastive = representation_contrastive + ((1 / (i+1)) * (loss_contrastive.item() - representation_contrastive))
				except Exception as e:
					print(e)
				...
				loss = lambda_loss * loss_contrastive + representation_loss_cosine + representation_loss_rmse
