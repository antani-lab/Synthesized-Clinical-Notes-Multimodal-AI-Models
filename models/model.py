...
class PanDerm_Encoder(nn.Module):
	def __init__(self, checkpoint_path, panderm_path):
		super().__init__()

		sys.path.append(panderm_path)
		from models.modeling_finetune import *

		self.model = panderm_base_patch16_224()
	...
	def forward(self, x):
		...
		x = self.model(x)  # shape: (B, num_tokens, hidden_dim)
...
class MultimodalArchitecture(torch.nn.Module):
	def __init__(self, device, CNN_TO_USE = 'densenet121', 
			  out_dim = 8, in_dim = 512, intermediate_dim=128, 
			  TEMPERATURE = 0.07, pretrained_path = None, patch_size = None, panderm_path = None):
		...
		elif (self.netcode is NETWORK.PanDerm):
			self.img_encoder = PanDerm_Encoder(pretrained_path, panderm_path)
