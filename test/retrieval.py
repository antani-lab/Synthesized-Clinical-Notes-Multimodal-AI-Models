...
MODEL_FLD = args.WEIGHTS + 'multimodal/' + MODALITY + '/'
...
def save_metric(MODEL_PATH, METRIC, DATASET, INPUT_DATA, OUTPUT_DATA, VALUE):


	fname_file = MODEL_PATH + METRIC + '_' + DATASET + '_input_' + INPUT_DATA + '_pool_' +OUTPUT_DATA+ '.csv'
...
def save_prediction(MODEL_PATH, K, DATASET, INPUT_DATA, OUTPUT_DATA, VALUE_lab):

	fname_file = MODEL_PATH + DATASET + '_input_' + INPUT_DATA + '_pool_' +OUTPUT_DATA+ '_K_' + str(K) + '_similarities_label.csv'
	print(fname_file)
...
for DATASET in POOL_DATASETS_TEST:
	...
	#METRIC = "recall@"+str(k)
	#utils_retrieval.save_metric(MODEL_FLD, METRIC, DATASET, MOD_INPUT_str, r, REPORT_TYPE)
	#print("recall@"+str(k) + ": " + str(r))
	...
	METRIC = "mAP"
	save_metric(MODEL_FLD, METRIC, DATASET, DATA_INPUT_str, DATA_POOL_str, mAP)
	print("mAP: " + str(mAP))

	whole_array = get_similarities(filenames, query_labels, pool_labels, sorted_indices, sim_matrix, k = KK)

	save_prediction(MODEL_FLD, K, DATASET, DATA_INPUT_str, DATA_POOL_str, whole_array)
