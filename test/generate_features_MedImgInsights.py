...
features_dir = MAIN_FOLDER+DATASET+'/features_MedImageInsights/'
...
for i in tqdm(range(len(test_dataset))):
	...
	if (os.path.exists(features_filename) == False):
		...
		if (flag_img):
			...
			image = base64.encodebytes(read_image(sample_fname)).decode("utf-8")

			# Example inference
			images = [image]

			image_feat = classifier.encode(images = images)['image_embeddings']
