...
features_dir = MAIN_FOLDER+DATASET+'/features_MONET/'
...
for i in tqdm(range(len(test_dataset))):
	...
	if (os.path.exists(features_filename) == False):
		...
		if (flag_img):
			...
			image = Image.open(sample_fname).convert("RGB")
			...
			with torch.no_grad():
				# underlying CLIP-like image encoder
				image_embeds = model.get_image_features(**inputs)  # [1, D]

			# L2-normalize (common for retrieval)
			#image_embeds = torch.nn.functional.normalize(image_embeds, p=2, dim=-1)
			cls_np = image_embeds.cpu().numpy()[0]  # shape (D,)
			...
			inputs = processor(
				text=[input_txt],          # list of strings
				return_tensors="pt",
				padding=True,
				truncation=True,
			).to(device)

			# 2) Pass through text encoder
			with torch.no_grad():
				text_embeds = model.get_text_features(**inputs)   # shape [1, D]

			# 3) L2 normalize (recommended for cosine similarity / retrieval)
			#text_embeds = torch.nn.functional.normalize(text_embeds, p=2, dim=-1)
			cls_np = text_embeds.cpu().numpy()[0]  # shape (D,)
