...
parser.add_argument('-p', '--PARTITION', help='internal, external, dermoscopic, clinic, ',type=str, default='classes_matching')
parser.add_argument('-d', '--DATA_FOLDER', help='path of the folder where to images are stored',type=str, default='')
...
MAX_LEN = 1000000
whole_filenames = np.empty(MAX_LEN, dtype = "object")
y_true = np.empty(MAX_LEN)
y_pred = np.empty(MAX_LEN)
whole_similarities = np.empty((MAX_LEN, N_CLASSES))
cont = 0

whole_array = np.empty((MAX_LEN, N_CLASSES + 3), dtype = "object")
...
for DATASET in DATASETS:
    ...
    for i in tqdm(range(len(test_dataset))):
        ...
        whole_filenames[cont] = current_fname
        ...
        y_true[cont] = mapping_concept_to_idx(csv_metadata[row,idx_metadata], keywords_list, CONCEPT_TO_USE)

        similarities = similarity_classes[row,1:]
        #y_pred.append(np.argmax(similarities))
        y_pred[cont] = np.argmax(similarities)
        whole_similarities[cont] = similarities

        whole_array[cont,0] = current_fname
        whole_array[cont,1] = y_true[cont]
        whole_array[cont,2] = y_pred[cont]
        whole_array[cont,3:] = similarities

        cont = cont + 1

whole_filenames = whole_filenames[:cont]
y_true = y_true[:cont]
y_pred = y_pred[:cont]
whole_similarities = whole_similarities[:cont]

f1_score = sklearn.metrics.f1_score(y_true = y_true, y_pred = y_pred, average = 'weighted')

print(f1_score)

new_filename = OUTPUT_FLD + "f1_score_"+CONCEPT_TO_USE_str+"_"+PARTITION_TO_USE_str+".csv"
File = {'val' : [f1_score]}
df = pd.DataFrame(File,columns=['val'])
np.savetxt(new_filename, df.values, fmt='%s',delimiter=',')

new_filename = OUTPUT_FLD + "f1_score_"+CONCEPT_TO_USE_str+"_"+PARTITION_TO_USE_str+"_similarities.csv"
np.savetxt(new_filename, whole_array, delimiter=",", fmt='%s',)